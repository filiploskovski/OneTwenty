...
public class JobsMiddleware
{
    ...
    {
        _recurringJobManager.AddOrUpdate<IRestApiJobService>("From Rest API",  x => x.Execute() , Cron.Hourly());
        _recurringJobManager.AddOrUpdate<ICsvJobService>("From CSV", (x) => x.Execute() , Cron.Hourly());
        await _next(context);
    }
}
